...
Use gh to locate failing PR checks, fetch GitHub Actions logs, summarize failure context, then propose a fix plan and implement after explicit approval.
...
1. Verify gh authentication.
   - Run `gh auth status` in the repo with workflow or repo scopes after running `gh auth login`.
...
6. Create a plan.
   - Use the plan tool to draft a concise fix plan and request approval.
